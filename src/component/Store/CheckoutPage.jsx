	...
	const handleSubmit = async (e) => {
		...
		const updateToSheet = async () => {
			let data = await fetch(
				`https://club-community-feedbox2-0-sdcn.vercel.app/submitProduct/${window.location.pathname.split("/")[
...
export const CheckoutPage = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			const productData = await fetch(`https://club-community-feedbox2-0-sdcn.vercel.app/getProduct/${id}/`);
	...
	useEffect(() => {
		const fetchCoin = async () => {
			const coinData = await fetch(`https://club-community-feedbox2-0-sdcn.vercel.app/getCoins`, {
				headers: {
					Authorization: "Bearer " + localStorage.getItem("jwt"),
				},
			});
